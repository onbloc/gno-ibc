...
| Send-side native escrow | [SendRaw](transfer.gno) direct user calls let [Send](v1/impl.gno) read attached native coins via `unsafe.OriginSend()` and release proxy-held escrow through [sendNative](v1/escrow.gno). | [send funds capture](https://github.com/unionlabs/union/blob/edaacacccc3544d69ce1fac0aa1c7e9b6fe83216/cosmwasm/app/ucs03-zkgm/src/contract.rs#L3021-L3036), [token-order v2 escrow funds](https://github.com/unionlabs/union/blob/edaacacccc3544d69ce1fac0aa1c7e9b6fe83216/cosmwasm/app/ucs03-zkgm/src/contract.rs#L419-L542) | Supported for direct EOA `SendRaw`/`Send`; Gno uses the proxy escrow account plus `unsafe.OriginSend()` instead of CosmWasm `MessageInfo::funds`. |
