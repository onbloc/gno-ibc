...
def ensure_clone(version: GnoVersion, cache_dir: Path) -> None:
    ...
    Always force-resets the working tree to the pinned commit before
    proceeding: a prior run leaves ``gnovm/stdlibs/generated.go`` modified
    in place (regenerated by ``go generate``) and untracked symlink
    directories under ``gnovm/stdlibs/<module>/`` for every package it
    linked. If a stdlib is later removed from this repo, the untracked
    directory and the matching import in ``generated.go`` both remain in
    the cache. CI's ``restore-keys`` cache fallback then resurrects that
    state on a key miss, and ``go mod tidy`` chokes on the dangling import.
    """
    ...
    # `-f` discards any tracked-file modifications left by a prior `go
    # generate`; harmless no-op when the tree is already pristine.
    run(["git", "checkout", "--quiet", "-f", version.commit], cwd=cache_dir)
    if not freshly_cloned:
        # Scrub the previous run's untracked symlink trees and gitignored
        # build artifacts. Skipped on fresh clones — the tree is pristine
        # and the full-repo scan is the most expensive step here.
        run(["git", "clean", "-fdx", "--quiet"], cwd=cache_dir)
