...
ZKGM wire bytes use Solidity ABI-compatible encoding through
`gno.land/p/core/encoding/abi`. Fixture generation uses the `abi_encode_params`
flavor described in
[Fixtures and Wire Compatibility](fixtures-and-wire-compatibility.md).

The outer packet envelope is:
...
`TAG_ACK_FAILURE` is zero. `TAG_ACK_SUCCESS` is a 32-byte value whose last byte
is `0x01`. `ACK_ERR_ONLY_MAKER` is the 4-byte marker `0xdeadc0de`.

`FILL_TYPE_PROTOCOL` ends in `0x00b0cad0`. `FILL_TYPE_MARKETMAKER` ends in
`0xd1cec45e`.
