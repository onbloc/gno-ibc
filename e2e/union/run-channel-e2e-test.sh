...
render_args="$test_dir/render-args"
rendered_legacy_config="$test_dir/voyager-config.json"
real_jq=$(command -v jq)
mkdir "$render_bin"
cat >"$render_bin/jq" <<'FAKE_JQ'
#!/usr/bin/env bash
printf '%s\n' "$@" >>"$RENDER_ARGS"
exec "$REAL_JQ" "$@"
FAKE_JQ
chmod 700 "$render_bin/jq"
render_key=0x$(printf '%064d' 0)
PATH="$render_bin:$PATH" REAL_JQ="$real_jq" RENDER_ARGS="$render_args" \
  EVM_CHAIN_ID=17000 EVM_IBC_HANDLER=0x1111111111111111111111111111111111111111 \
  EVM_MULTICALL=0x2222222222222222222222222222222222222222 \
  TRUSTED_MPT_PRIVATE_KEY="$render_key" UNION_PRIVATE_KEY="$render_key" \
  EVM_PRIVATE_KEY="$render_key" GNO_PRIVATE_KEY="$render_key" \
  VOYAGER_CONFIG_OUTPUT="$rendered_legacy_config" "$script_dir/render-voyager-config.sh" >/dev/null
jq -e '.modules.state[] | select(.info.chain_id=="17000") |
  .config.ibc_handler_address=="0x1111111111111111111111111111111111111111"' \
  "$rendered_legacy_config" >/dev/null
jq -e '.plugins[] | select(.config.multicall_address?) |
  .config.multicall_address=="0x2222222222222222222222222222222222222222"' \
  "$rendered_legacy_config" >/dev/null
[[ $(stat -f '%Lp' "$rendered_legacy_config" 2>/dev/null || stat -c '%a' "$rendered_legacy_config") == 600 ]]
if grep -Eq '__[A-Z0-9_]+__' "$rendered_legacy_config" || grep -Fq "$render_key" "$render_args"; then
  echo 'legacy Voyager renderer leaked a secret argument or placeholder' >&2
  exit 1
fi
if EVM_CHAIN_ID='' VOYAGER_CONFIG_OUTPUT="$rendered_legacy_config" \
  "$script_dir/render-voyager-config.sh" >"$test_dir/render-missing.out" 2>&1; then
  echo 'legacy renderer accepted a missing EVM chain ID' >&2
  exit 1
fi
grep -q 'EVM_CHAIN_ID must be a positive decimal integer' "$test_dir/render-missing.out"
if EVM_CHAIN_ID=17000 EVM_IBC_HANDLER=bad VOYAGER_CONFIG_OUTPUT="$rendered_legacy_config" \
  "$script_dir/render-voyager-config.sh" >"$test_dir/render-malformed.out" 2>&1; then
  echo 'legacy renderer accepted a malformed EVM address' >&2
  exit 1
fi
grep -q 'EVM_IBC_HANDLER must be a 20-byte hex address' "$test_dir/render-malformed.out"
if COMPOSE_PROJECT_NAME=test VOYAGER_CONTAINER=test UNION_CONTAINER=test UNION_CORE_CONTRACT=test \
  EVM_CHAIN_ID=bad EVM_IBC_HANDLER=0x1111111111111111111111111111111111111111 \
  "$script_dir/setup-clients.sh" >"$test_dir/setup-client-chain.out" 2>&1; then
  echo 'client setup accepted a malformed EVM chain ID' >&2
  exit 1
fi
grep -q 'EVM_CHAIN_ID must be a positive decimal integer' "$test_dir/setup-client-chain.out"
if COMPOSE_PROJECT_NAME=test VOYAGER_CONTAINER=test UNION_CONTAINER=test UNION_CORE_CONTRACT=test \
  EVM_CHAIN_ID=17000 EVM_IBC_HANDLER=bad \
  "$script_dir/setup-clients.sh" >"$test_dir/setup-client-address.out" 2>&1; then
  echo 'client setup accepted a malformed EVM handler' >&2
  exit 1
fi
grep -q 'EVM_IBC_HANDLER must be a 20-byte hex address' "$test_dir/setup-client-address.out"
if VOYAGER_CONTAINER=test EVM_CHAIN_ID=17000 \
  EVM_ZKGM_CONTRACT=0x3333333333333333333333333333333333333333 \
  GNO_EVM_CLIENT_ID=bad EVM_GNO_CLIENT_ID=2 \
  "$script_dir/setup-gno-evm-topology.sh" >"$test_dir/setup-topology-client.out" 2>&1; then
  echo 'topology setup accepted a malformed client ID' >&2
  exit 1
fi
grep -q 'GNO_EVM_CLIENT_ID must be a positive decimal integer' "$test_dir/setup-topology-client.out"
if VOYAGER_CONTAINER=test EVM_CHAIN_ID=17000 EVM_ZKGM_CONTRACT=bad \
  GNO_EVM_CLIENT_ID=1 EVM_GNO_CLIENT_ID=2 \
  "$script_dir/setup-gno-evm-topology.sh" >"$test_dir/setup-topology-address.out" 2>&1; then
  echo 'topology setup accepted a malformed ZKGM address' >&2
  exit 1
fi
grep -q 'EVM_ZKGM_CONTRACT must be a 20-byte hex address' "$test_dir/setup-topology-address.out"
