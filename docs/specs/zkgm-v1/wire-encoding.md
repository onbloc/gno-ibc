...
The diagrams below assume a monospace rendering. Each `+--+` boundary is one
byte; `/` borders mark dynamic-field tails.

## Encoding invariants

| Invariant | Notes |
|-----------|-------|
| ABI flavor | Solidity params tuple (`abi.encode_params(...)`). Plain `abi.encode(...)` for a single dynamic top-level value is not produced or accepted. |
...
| `FORWARD_SALT_MAGIC` | `c0de` at bytes 0-1, `babe` at bytes 30-31, zeros elsewhere | first and last byte pairs | OR-masked into forwarded child salt so `IsForwardedPacket` can detect it |
