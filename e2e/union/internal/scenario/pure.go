...
func packetOutcomeError(outcome state.PacketOutcome) error {
	...
	case state.PacketOutcomeFailure:
		return errors.New("packet failure acknowledgement and escrow refund verified")
	default:
		return errors.New("completed packet has invalid outcome")
	}
}

func matchingAcknowledgementResult(gno, evm string) (bool, error) {
	gnoBytes, gnoSuccess, err := parseAcknowledgement(gno)
	if err != nil {
		return false, errors.New("malformed Gno acknowledgement")
	}
	evmBytes, evmSuccess, err := parseAcknowledgement(evm)
	if err != nil {
		return false, errors.New("malformed EVM acknowledgement")
	}
	if gnoSuccess != evmSuccess {
		return false, errors.New("Gno and EVM acknowledgement results differ")
	}

	if !bytes.Equal(gnoBytes, evmBytes) {
		return false, errors.New("Gno and EVM acknowledgements differ")
	}

	return gnoSuccess, nil
}

func parseAcknowledgement(value string) ([]byte, bool, error) {
	value = strings.TrimPrefix(value, "0x")
	if value == "" || len(value)%2 != 0 {
		return nil, false, errors.New("malformed acknowledgement")
	}
...
func classifyBoundaryBalances(
	...
	if err != nil {
		return state.Balances{}, errors.New("Gno voucher balance is malformed")
	}
